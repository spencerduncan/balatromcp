...
class TestReadActionResult:
    ...
    @pytest.mark.asyncio
    async def test_read_action_result_cleanup_on_read(
        self, file_io, sample_action_result
    ):
    ...
    @pytest.mark.asyncio
    async def test_read_action_result_cleanup_error_ignored(
        self, file_io, sample_action_result
    ):
    ...
    @pytest.mark.asyncio
    async def test_read_action_result_sequence_tracking(
        self, file_io, sample_action_result
    ):
...
class TestWaitForActionResult:
    ...
    @pytest.mark.asyncio
    async def test_wait_for_action_result_delayed_appearance(
        self, file_io, sample_action_result
    ):
        ...
        with patch("pathlib.Path.exists", return_value=True):
            with patch("pathlib.Path.stat", return_value=mock_stat):
                with patch("pathlib.Path.unlink") as mock_unlink:
                    ...
                    with patch.object(
                        file_io, "_validate_file_operation", return_value=None
                    ):
...
class TestPathValidationSecurity:
    ...
    def test_validate_and_sanitize_path_directory_traversal_blocked(self):
        ...
        for pattern in traversal_patterns:
            with pytest.raises(
                ValueError, match="Directory traversal|invalid or potentially dangerous"
            ):
    ...
    def test_validate_and_sanitize_path_absolute_paths(self):
        ...
        with pytest.raises(
            ValueError, match="outside allowed directories|escape working directory"
        ):
...
class TestSecurityIntegration:
    ...
    async def test_cleanup_old_files_security_validation(self, temp_path):
        ...
        with patch.object(
            file_io, "_validate_file_operation", side_effect=mock_validate
        ):
            with patch("pathlib.Path.exists", return_value=True):
...
class TestSecurityLogging:
    ...
    def test_path_validation_logs_security_violations(self, caplog):
        ...
        assert (
            "Blocked path pattern detected" in caplog.text
            or "Directory traversal attempt" in caplog.text
        )
